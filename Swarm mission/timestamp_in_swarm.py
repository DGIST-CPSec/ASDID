...
leader_orientations = {'roll': [], 'pitch': [], 'yaw': []}
follower_orientations = {'roll': [], 'pitch': [], 'yaw': []}
follower_receive_times = []
...
def position_callback(timestamp, data, logconf):
    ...
    leader_orientations['roll'].append(roll)
    leader_orientations['pitch'].append(pitch)
    leader_orientations['yaw'].append(yaw)
    follower_receive_times.append(time.time())
...
def follower_position_callback(timestamp, data, logconf):
    ...
    follower_orientations['roll'].append(roll)
    follower_orientations['pitch'].append(pitch)
    follower_orientations['yaw'].append(yaw)
...
logconf.add_variable('stateEstimate.roll', 'float')   # 롤, 피치, 요 로그 변수 추가
...
def save_to_csv(filename, data):
    with open(filename, 'w', newline='') as csvfile:
        ...
        writer.writeheader()
        for i in range(len(data['x'])):  # 'x'를 기준으로 loop를 돌립니다.
            writer.writerow({k: data[k][i] for k in fieldnames})
            
def save_follower_receive_times_to_csv(filename, receive_times):
    with open(filename, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['시간 간격 (초)'])
        
        for i in range(1, len(receive_times)):
            time_interval = receive_times[i] - receive_times[i - 1]
...
if __name__ == '__main__':
    ...
    save_to_csv("leader_orientations.csv", leader_orientations)
    save_to_csv("follower_orientations.csv", follower_orientations)
    save_follower_receive_times_to_csv("follower_receive_times.csv", follower_receive_times)
